...
# 时间轴：以波1峰值为原点，显示±1个100kHz周期（共2个周期=20μs）
t_range = 1 * (1/fs)  # 单个周期10μs，显示±10μs（共2周期）
t = np.linspace(-t_range, t_range, 2000)  # 高密度采样（2000点，确保竖线平滑）
...
def generate_wave_segments(freq, amp, delay, t):
    ...
    for i, tj in enumerate(jump_times):
        ...
        t_start = tj if i == 0 else jump_times[i-1]
        segments["horizontal"].append((t_start, tj, current_level, 3.0))  # 水平线粗3pt
    ...
    # 最后一个区间（最后跳变点→时间轴终点）
    t_start = jump_times[-1]
    segments["horizontal"].append((t_start, t.max(), current_level, 3.0))
...
# ---------------------- 图形美化 ----------------------
plt.title('Two-Cycle Square Waves (Thick Horizontal, Thin Vertical Lines)', fontsize=14)
plt.xlabel('Time relative to Wave 1 peak (μs)', fontsize=12)  # x轴单位μs
